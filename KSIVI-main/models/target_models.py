...
class Star_shaped(Toy_2D):
    name = "star_shaped"
    def __init__(self, device):
        self.device = device
        super().__init__("Star_shaped")
    def logp(self, X):
        det = 3.8 * 0.2
        sigmasqinv_0 = torch.tensor([[2., 1.8], [1.8, 2.]]).to(self.device) / det
        sigmasqinv_1 = torch.tensor([[3.8, 0], [0, 0.2]]).to(self.device) / det
        sigmasqinv_2 = torch.tensor([[2., -1.8], [-1.8, 2.]]).to(self.device) / det
        sigmasqinv_3 = torch.tensor([[0.2, 0], [0, 3.8]]).to(self.device) / det # shape = [1, d, d]
        
        return -0.5 * 2 * np.log(2 * np.pi) - 0.5 * np.log(det * 16) + torch.logsumexp(torch.stack(
            (-1/2 * torch.matmul(torch.matmul(X[:,None,:],sigmasqinv_0), X[:,:,None]).squeeze(-1),
    ...
    def score(self, X):
        ...
        return -Y[:,0] * torch.matmul(sigmasqinv_0, X[:,:,None]).squeeze(-1) - Y[:,1] * torch.matmul(sigmasqinv_1, X[:,:,None]).squeeze(-1) - Y[:,2] * torch.matmul(sigmasqinv_2, X[:,:,None]).squeeze(-1) - Y[:,3] * torch.matmul(sigmasqinv_3, X[:,:,None]).squeeze(-1)

    # def Hessian(self, X):
    #     det = 3.95 * 0.05
    #     sigmasqinv_0 = torch.tensor([[3.95, 0.], [0., 0.05]]).to(self.device) / det
    #     sigmasqinv_1 = torch.tensor([[2., 1.95], [1.95, 2.]]).to(self.device) / det
    #     sigmasqinv_2 = torch.tensor([[0.05, 0.], [0., 3.95]]).to(self.device) / det
    #     sigmasqinv_3 = torch.tensor([[2., -1.95], [-1.95, 2.]]).to(self.device) / det
    ...
    #     return -Y[:,0] * sigmasqinv_0 - Y[:,1] * sigmasqinv_1 - Y[:,2] * sigmasqinv_2 - Y[:,3] * sigmasqinv_3
