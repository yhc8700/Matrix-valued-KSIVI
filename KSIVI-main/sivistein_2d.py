...
class KernelSIVI(object):
    def __init__(self, config):
        ...
        self.kernel = {"gaussian":gaussian_kernel, "laplace":laplace_kernel,
                       "IMQ":IMQ_kernel, "Riesz": Riesz_kernel, "matrix": matrix_kernel}[self.config.kernel]
    ...
    def learn(self):
        ...
        for epoch in tqdm(range(1, self.trainpara.num_epochs+1)):
            ...
            if (epoch - 1)%self.config.sampling.visual_time ==0 or epoch ==self.trainpara.num_epochs:
                if self.target in ["banana", "multimodal", "x_shaped", "star_shaped"]:
                    save_to_path = os.path.join(self.save_fig_to_path, str(self.iter_idx)+'.png')
                    bbox = {"multimodal":[-5, 5, -5, 5], "banana":[-3.5,3.5,-6,1], "x_shaped":[-5,5,-5,5], "star_shaped":[-5,5,-5,5]}
                    quiver_plot = False
            ...
            for i in range(1, self.trainpara.num_perepoch+1):
                ...
                if not self.trainpara.ustat:
                    if self.config.kernel == "matrix":
                        ...
                        f1 = (compu_targetscore + neg_score_implicit) # tensor with shape (n,d)
                        f2 = (compu_targetscore_aux  + neg_score_implicit_aux).unsqueeze(0).unsqueeze(3) # tensor with shape (1,n,d,1)
                        k = self.kernel(X, X_aux, detach=self.trainpara.detach) # tensor with shape (n,n,d,d)
                        tmp = torch.matmul(f1.unsqueeze(1).unsqueeze(2), torch.matmul(k, f2)) # tensor with shape (n,n),
                        loss_kxy = tmp.mean()
